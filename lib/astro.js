...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "ITACHI_x_RAGE"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "ITACHI_x_RAGE") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "ITACHI_x_RAGE") => {
...
app.listen(randomPort, () =>
  console.log(
    "ITACHI_x_RAGE Server listening on http://localhost:" + randomPort + "/ "
  )
);
