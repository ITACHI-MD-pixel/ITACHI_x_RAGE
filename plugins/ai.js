const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*ITACHI_x_RAGE ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const astro = "ITACHI_x_RAGE ɢᴘᴛ𝟺\n";
      const tbl = "```";
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "ITACHI_x_RAGE ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
  ...
  smd(
    ...
    async (_0x1c0160, _0x482db1) => {
        let astro = "ITACHI_x_RAGE ᴄʜᴀᴛ ʙᴏᴛ"
      try {
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("ITACHI_x_RAGE• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("ITACHI_x_RAGE• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*ITACHI_x_RAGE • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
