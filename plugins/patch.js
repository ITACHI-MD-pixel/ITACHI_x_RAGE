  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`ITACHI_x_RAGE- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing ITACHI_x_RAGE
│└────────────┈ ⳹        
  ...
  async (_0x22514a) => {
    try {
      let _0x20ed34 = (
        ...
        "/" +
        formatp(os.totalmem()) +
        "\n\n *👨‍💻 :*  ITACHI_x_RAGE ɪꜱ ɴᴏᴡ Aᴠᴀɪʟᴀʙʟᴇ\n\n" +
        readmore +
        "\n╭──❰ *ALL MENU* ❱\n│🏮 Lɪꜱᴛ\n│🏮 Cᴀᴛᴇɢᴏʀʏ\n│🏮 Hᴇʟᴘ\n│🏮 Aʟɪᴠᴇ\n│🏮 Uᴘᴛɪᴍᴇ\n│🏮 Wᴇᴀᴛʜᴇʀ\n│🏮 Lɪɴᴋ\n│🏮 Cᴘᴜ\n│🏮 Rᴇᴘᴏꜱɪᴛᴏʀʏ\n╰─────────────⦁"
      ).trim();
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*ITACHI_x_RAGE ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
      ...
      const designs = [
        async () => {
          const imageBuffer = await axios.get(
            "https://i.imgur.com/Tgam36h.jpeg",
            {
          ...
          const captionText = `ITACHI_x_RAGE 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nITACHI_x_RAGE`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*ITACHI_x_RAGE ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "ITACHI_x_RAGE Running Since",
        body: message,
        footerText: "ITACHI_x_RAGE 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("ITACHI_x_RAGE_FANCY_TEXT")
        ) {
